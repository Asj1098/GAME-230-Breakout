...
int Ball::UpdatePosition(float delta_time, sf::Vector2u window_size)
{
    int hit_wall = 0;
...
void Ball::Reflect(int directionX, int directionY)
{
    if(directionX!=0)
        velocity.x = directionX * abs(velocity.x);;
    if (directionY != 0)
        velocity.y = directionY * abs(velocity.y);;
